...
namespace Beamable.Spoons
{
	...
	public class Spoons : Microservice
	{
		...
		[ClientCallable]
		public async Promise<Convo> StartChat(string message)
		{
			var oai = Provider.GetService<OpenAI>();
			var promptGen = Provider.GetService<PromptService>();

			var convo = new Convo
			{
				openingPrompt = promptGen.GetChatGpt35TurboPrompt(),
				id = Guid.NewGuid().ToString(),
				userMessages = new List<ConvoUserMessage>(new ConvoUserMessage[]
				{
			...
			var _ = oai.SendChat(convo);

			return convo;
		}
		
		[ClientCallable]
		public Promise<Convo> Start(string message)
		{
